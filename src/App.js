...
class App extends Component{
  constructor(props){
    ...
    getFilteredImages=()=>{
      ...
      const filteredimages=imagesList.filter((eachitem)=>(
    ...
    selectImage=(imageUrl)=>{
      ...
      this.setState({
        randomImage:imagesList[Math.ceil(Math.random()*len)].imageUrl
      })
  ...
  render(){
    const{score,time,activeTabId,randomImage,playagain}=this.state
    ...
    return(
      <div className='maincontainer'>
        ...
        {time===0 && playagain===false ?<div><Scorecard gamescore={score} onClickPlayAgain={this.onClickPlayAgain} /></div>:
        <div>
        ...
        <div className='container2'>
          {tabsList.map((tabdetails)=>(
            <Items key={tabdetails.tabId} tabdetails={tabdetails} clickTabItem={this.clickTabItem} isActive={activeTabId===tabdetails.tabId}/>
          ))}
        </div>
        <div className='container3'>
            {filteredimages.map((details)=>(
              <Images details={details} selectImage={this.selectImage}/>
            ))}
        </div>
